...
public class SmartObjectGui extends GuiScreen {
    ...
    public SmartObjectGui(SmartObject smartObject) {
        this.smartObject = smartObject;
    }

    public SmartObjectGui(SmartObject smartObject, GuiScreen lastGui) {
        this(smartObject);
        this.lastGui = lastGui;
        for (SmartVariable smartVariable : smartObject.getConnectedVariables()) {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        super.drawCenteredString(this.fontRendererObj, this.smartObject.getCustomDisplayString(), Video.getScaledWidth(150), Video.getScaledHeight(60), 0xffffffff);
        ...
        if (this.displayedButtons.size() != 0) {
            ...
            if (((j > 0) && this.scrollBar.yPosition > 0) || ((j < 0) && this.scrollBar.yPosition + this.scrollBar.height < this.height)) {
                ...
                for (GuiButton guiButton : SmartObjectGui.this.buttonList) {
    ...
    public void initGui() {
        this.searchField = new SmartTextField(0, Video.getScaledWidth(75), this.height - 40, Video.getScaledWidth(150), 20, "Search") {
            ...
            public void handleInput() {
                SmartObjectGui.this.listComponents();
            }
        };
        ...
        if (this.lastGui != null) {
            this.buttonList.add(new SmartButton("Back", 5, 5, 5, 50) {
                @Override
                public void work() {
                    Core.openGui(SmartObjectGui.this.lastGui);
                }
            });
        }
        this.listComponents();
    ...
    private void listComponents() {
        ...
        for (SmartVariable smartVariable : this.smartObject.getConnectedVariables()) {
                ...
                if (smartVariable instanceof SmartBoolean) {
                        ...
                        this.buttonList.add(open = new SmartButton(componentId,
                            ...
                            public void work() {
                                if (((SmartBoolean) smartVariable).getList() instanceof SmartList<?>) {
                                    Core.openGui(new ListGui((SmartList<?>) ((SmartBoolean) smartVariable).getList(), SmartObjectGui.this));
                                } else {
                                    Core.openGui(new SmartObjectListGui((SmartObjectList<?>) ((SmartBoolean) smartVariable).getList(), SmartObjectGui.this));
                                }
                            }
                        });
                    ...
                    if (!(smartVariable instanceof SmartNumber.SmartLong)) {
                        ...
                        this.buttonList.add(new SmartButton(componentId + 1, Video.get2ndPanelCenter() + 110,
                            ...
                            @Override
                            public void work() {
                                SmartObjectGui.this.smartTextFields.get(i).handleInput();
                            }
                        });
                        nextYPosition += 30;
                        componentId += 2;
                    ...
                    this.displayedButtons.add(new SmartButton.SmartVariableButton(componentId, nextYPosition, smartVariable) {
                        @Override
                        public void work() {
                            Core.openGui(new ListGui((SmartList<?>) smartVariable, SmartObjectGui.this));
                        }
                    });
                    nextYPosition += 30;
                    componentId++;
                    ...
                    this.displayedButtons.add(new SmartButton(componentId, nextYPosition, smartVariable.getCustomDisplayString()) {
                        @Override
                        public void work() {
                            Core.openGui(new SmartObjectGui((SmartObject) smartVariable, SmartObjectGui.this));
                        }
                    });
                    nextYPosition += 30;
                    componentId++;
                    ...
                    this.displayedButtons.add(new SmartButton(componentId, nextYPosition, smartVariable.getCustomDisplayString()) {
                        @Override
                        public void work() {
                            Core.openGui(new SmartObjectListGui((SmartObjectList<?>) smartVariable, SmartObjectGui.this));
                        }
                    });
                    nextYPosition += 30;
                    componentId++;
        ...
        for (int i = 0; i < this.groupNames.size(); i++) {
            ...
            this.buttonList.add(new SmartButton(this.buttonList.size() - 1, Video.getScaledWidth(100), Video.getScaledHeight(150) + 25 * i, Video.getScaledWidth(100), 20, this.groupNames.get(i)) {
                final String name = SmartObjectGui.this.groupNames.get(finalI);

                @Override
                public void work() {
                    SmartObjectGui.this.selectedGroup = this.name;
                    SmartObjectGui.this.listComponents();
                }
                ...
                public void drawButton(Minecraft minecraft, int mouseX, int mouseY) {
                    if (this.visible) {
                        ...
                        this.displayString = (SmartObjectGui.this.selectedGroup.equals(this.name) ? MOD_COLOR + "" + BOLD : "") + this.name;
        ...
        this.buttonList.add(this.scrollBar = new ScrollBar() {
            ...
            public void onDrag(int scroll) {
                ...
                int scaledScroll = (int) (scroll * SmartObjectGui.this.panelHeight / (double) SmartObjectGui.this.height);

                while (scaledScroll > 0 && SmartObjectGui.this.getFirstComponentOriginalYPosition() + scaledScroll > 10) {
                    scaledScroll--;
                }

                while (scaledScroll < 0 && SmartObjectGui.this.getLastComponentOriginalYPosition() + 20 + scaledScroll < SmartObjectGui.this.height - 10) {
                    scaledScroll++;
                }


                for (GuiButton guiButton : SmartObjectGui.this.displayedButtons) {
                    if (guiButton instanceof SmartButton) {
                ...
                for (SmartTextField smartTextField : SmartObjectGui.this.smartTextFields) {
